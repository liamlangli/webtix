...
export class Renderer {
    ...
    constructor(public canvas: HTMLCanvasElement) {
        ...
        const debug = this.device.getExtension<WEBGL_debug_renderer_info>(
            "WEBGL_debug_renderer_info",
        );

        let performanceScale = 1.0;
        if (debug) {
            const unmask_renderer = context.getParameter(
    ...
    clear(colorMask: boolean, depthMask: boolean, stencilMask: boolean): void {
        ...
        if (colorMask) gl.clearColor(color.R, color.G, color.B, color.A);
