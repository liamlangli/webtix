 ...
 * @param position
 */
export function bvh_build_geometry_indexed(
    ...
    console.log(
        `[bvh_build_geometry_indexed] build bvh tree with number of ${triangleCount} triangles`,
    );
    ...
    console.assert(
        bvh_node_count === count,
        `incorrect bvh node count ${bvh_node_count} ${count}.`,
    );

    const result = new Float32Array(
        bvh.buffer,
        0,
        bvh_node_count * BUFFER_STRIDE_BVH_NODE,
    );
    boxes = undefined;
    bvh = undefined;
    ...
    console.log(
        `[bvh_build_geometry_indexed] bvh build finished, cost ${duration.toFixed(3)}ms`,
    );
...
function bvh_node_compare_func_axis(a: number, b: number): number {
    return (
        boxes![a * BUFFER_STRIDE_BOX + BUFFER_BOX_OFFSET_CENTER + axis] -
        boxes![b * BUFFER_STRIDE_BOX + BUFFER_BOX_OFFSET_CENTER + axis]
    );
}
...
function bvh_split_balanced(from: number, to: number): number {
    ...
    bvh![node_index * BUFFER_STRIDE_BVH_NODE + BUFFER_OFFSET_BVH_COUNT] =
        left + right;
