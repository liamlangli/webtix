...
export function draco_get_attribute(
    geometry: DracoGeometry,
    name: string,
): DracoNamedAttribute | undefined {
...
export function draco_set_attribute(
    geometry: DracoGeometry,
    name: string,
    array: BufferArray,
    itemSize: number,
...
async function initDecoder(): Promise<any> {
    ...
    const useJS =
        typeof WebAssembly !== "object" || decoderConfig.type === "js";
    const librariesPending = [];
    if (useJS) {
        librariesPending.push(
            loadLibrary("draco_decoder.js", ResponseType.Text),
        );
    } else {
        librariesPending.push(
            loadLibrary("draco_wasm_wrapper.js", ResponseType.Text),
        );
        librariesPending.push(
            loadLibrary("draco_decoder.wasm", ResponseType.ArrayBuffer),
        );
    }

    decoderPending = Promise.all(librariesPending).then((libraries) => {
        ...
        if (workerPool.length < workerLimit) {
            ...
            worker.onmessage = function (e: MessageEvent) {
                ...
                switch (message.type) {
                    ...
                    default:
                        console.error(
                            `DRACOLoader: Unexpected message, ${message.type}`,
                        );
                }
            };
            ...
            case "decode":
                ...
                decoderPending.then((module: any) => {
                    ...
                    try {
                        ...
                        var buffers = geometry.attributes.map(
                            (attr: any) => attr.array.buffer,
                        );
                        if (geometry.index)
    ...
    function decodeAttribute(
        ...
        switch (attributeType) {
            ...
            case Int8Array:
                ...
                (self as any).geometryBuffer[attributeName] = new Int8Array(
                    draco.HEAP8.buffer,
                    ptr,
                    numValues,
                ).slice();
            ...
            case Uint8Array:
                ...
                (self as any).geometryBuffer[attributeName] = new Uint8Array(
                    draco.HEAPU8.buffer,
                    ptr,
                    numValues,
                ).slice();
